...
class NaverLoginMixin:
    ...
    def login_with_naver(self, state, code):
        ...
        if created:  # 사용자 생성할 경우
            ...
            selected = ""
            setting = user.setting_set.create(unit=10, max_value=10000, min_value=5000, weight=2.0,
                                              selected=selected)
            setting.save()

            monitoring = user.monitoring_set.create(query_count=0, setting_count=0)
            monitoring.save()

            user.save()
            # self.model.objects.getor

        # 로그인
        # login(self.request, user, 'user.oauth.backends.NaverBackend')  # NaverBackend 를 통한 인증 시도
        login(self.request, user)  # NaverBackend 를 통한 인증 시도
